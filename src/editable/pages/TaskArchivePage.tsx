...
function EditorialArchive({
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-white text-[#08080c]">
        <section className="border-b border-[var(--slot4-line)] bg-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-14">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.24em] text-[var(--slot4-accent)]">Media catalog</p>
              <h1 className="mt-3 text-5xl font-normal leading-none tracking-[-0.055em] sm:text-7xl lg:text-8xl">
                {category === 'all' ? label : categoryLabel}
              </h1>
            </div>
            <p className="max-w-md border-l-4 border-[var(--slot4-accent)] pl-5 text-sm leading-7 text-black/65">
              Press releases, announcements, and media-ready updates organized for fast discovery.
            </p>
          </div>
        </section>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[var(--editable-container)] border-x border-[var(--slot4-line)] bg-white lg:grid-cols-[1.75fr_0.75fr]">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden border-b border-[var(--slot4-line)] lg:border-b-0 lg:border-r">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="rounded-full bg-[var(--slot4-accent)] px-4 py-2 text-[10px] font-bold uppercase tracking-[0.16em]">{getCategory(lead, label)}</span>
                <h2 className="mt-5 max-w-4xl text-4xl font-normal leading-[1.02] tracking-[-0.045em] sm:text-6xl">{lead.title}</h2>
                <p className="mt-5 max-w-2xl line-clamp-2 text-sm font-semibold leading-7 text-white/80">{getSummary(lead)}</p>
              </div>
            </Link>
            <div className="grid">
              <div className="border-b border-[var(--slot4-line)] bg-[var(--slot4-accent-soft)] p-6 text-black">
                <p className="text-xs font-black uppercase tracking-[0.24em]">Top stories</p>
                <p className="mt-3 text-3xl font-normal leading-tight">What distribution teams are watching now.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] border-b border-[var(--slot4-line)] bg-white last:border-b-0">
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="mt-3 text-xl font-normal leading-tight">{post.title}</h3>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] border-x border-[var(--slot4-line)] bg-white px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b border-[var(--slot4-line)] pb-4">
            <h2 className="text-4xl font-normal tracking-[-0.04em] sm:text-5xl">More from the desk</h2>
            <form action={basePath} className="flex overflow-hidden rounded-full border border-[var(--slot4-line)] bg-white">
              ...
              <button className="h-11 bg-[var(--slot4-accent)] px-5 text-xs font-bold uppercase tracking-[0.14em] text-white">Filter</button>
            </form>
          </div>
          ...
          {remaining.length ? (
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group overflow-hidden rounded-xl border border-[var(--slot4-line)] bg-white transition hover:-translate-y-1 hover:shadow-[0_18px_44px_rgba(0,0,0,0.1)]">
                  <div className="aspect-[16/10] overflow-hidden bg-[var(--slot4-gray)]">
                    <img src={getImage(post)} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                  </div>
                  <div className="p-5">
                    <div className="flex items-center justify-between gap-4 text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--slot4-accent)]">
                      <span>{getCategory(post, label)}</span><span>{String(index + 3).padStart(2, '0')}</span>
                    </div>
                    <h3 className="mt-4 text-2xl font-normal leading-[1.08] tracking-[-0.035em]">{post.title}</h3>
            ...
            <div className="rounded-xl border border-dashed border-[var(--slot4-line)] bg-white p-12 text-center">
              <Search className="mx-auto h-8 w-8" />
              <h2 className="mt-4 text-3xl font-normal">No stories found</h2>
          ...
          <div className="mt-10 flex items-center justify-center gap-0">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-l-full border border-[var(--slot4-line)] bg-white px-5 py-3 text-xs font-bold uppercase">Previous</Link> : null}
            <span className="border-y border-[var(--slot4-line)] bg-[var(--slot4-accent)] px-5 py-3 text-xs font-bold uppercase text-white">Page {page} / {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-r-full border border-[var(--slot4-line)] bg-white px-5 py-3 text-xs font-bold uppercase">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
