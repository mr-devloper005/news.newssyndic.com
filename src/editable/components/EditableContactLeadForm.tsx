  ...
  return (
    <form onSubmit={handleSubmit} className="mt-8 border-t border-[var(--slot4-line)] bg-white pt-7">
      ...
      <label className="mt-4 grid gap-2 text-sm font-black opacity-75">
        Message
        <textarea name="message" required rows={6} placeholder="Tell us what you need help with..." className="rounded-xl border border-[var(--slot4-line)] bg-white px-4 py-3 text-base font-medium outline-none transition focus:border-[var(--slot4-accent)]" />
      </label>
      ...
      <button type="submit" disabled={status === 'submitting'} className="mt-6 inline-flex h-[3.25rem] w-full items-center justify-center gap-2 rounded-full bg-[var(--slot4-accent)] px-6 text-sm font-bold text-white transition hover:bg-black disabled:cursor-not-allowed disabled:opacity-70">
...
function Field({ name, label, type = 'text', placeholder, required = false }: { name: string; label: string; type?: string; placeholder?: string; required?: boolean }) {
  return (
    <label className="grid gap-2 text-sm font-black opacity-75">
      {label}
      <input name={name} type={type} required={required} placeholder={placeholder} className="h-[3.25rem] rounded-xl border border-[var(--slot4-line)] bg-white px-4 text-base font-medium outline-none transition focus:border-[var(--slot4-accent)]" />
    </label>
  )
}
